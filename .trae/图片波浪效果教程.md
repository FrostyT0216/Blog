...
| --- | --- | --- | --- |
| amplitude | number | 0.5 | Height of the waves (0 to 3). |
| speed | number | 0.65 | How fast the rings travel outward. 1 is normal speed. |
| wavelength | number | 80 | Distance between wave crests in CSS pixels. |
| rings | number | 2 | Number of crests in each wave train (1 to 8). |
| decay | number | 1 | How quickly the waves lose energy (higher dies faster). |
| refraction | number | 100 | How strongly the waves bend the page content, in CSS pixels. |
| dispersion | number | 0.5 | Chromatic dispersion splitting colors along the wave slopes (0 to 1). |
| shine | number | 0.5 | Intensity of the light glints on the wave crests (0 to 2). |
| trigger | "click" | "hover" | "none" | "click" | What spawns ripples. "click" on press, "hover" also leaves a wake while moving, "none" only ambient. |
| interval | number | 0 | Seconds between ambient ripples at random positions. 0 disables them. |
