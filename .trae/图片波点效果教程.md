...
```sh
npx shadcn@latest add @canvas-ui/asciify-react
```
...
export function Asciify(props: AsciifyProps) {
...
| --- | --- | --- | --- |
| radius | number | 0.4 | Radius of the ascii lens around the cursor, relative to the screen height. |
| softness | number | 1 | Edge feather of the lens as a fraction of the radius (0 to 1). |
| scale | number | 2 | Size of one glyph pixel in CSS pixels. Characters are 5x5 glyph pixels. |
| spacing | number | 1 | Empty glyph pixels around each character (0 to 3). |
| charset | "ascii" | "blocks" | "binary" | "ascii" | Built-in character ramp: real ascii glyphs, shade blocks, or binary digits. |
| glyphs | number[] | [] | Custom ramp of packed 5x5 bitmaps (dark to bright), overrides charset. |
| background | [number, number, number] | "auto" | [0, 0, 0] | Paper color behind the glyphs as [r, g, b] in 0-1 range, or "auto" to match the page background. |
| backgroundOpacity | number | 0 | Opacity of the background behind the glyphs (0 to 1). |
| contrast | number | 1 | Contrast applied to character density before picking a glyph. |
| brightness | number | 0 | Density offset applied before picking a glyph (-1 to 1). |
| invert | number | 0 | Invert character density inside the effect (0 to 1). |
| strength | number | 1 | Coverage of asciified cells inside the lens (0 to 1). |
| baseStrength | number | 0 | Ascii coverage across the whole screen, outside the lens (0 to 1). |
| followSpeed | number | 3 | How quickly the lens follows the cursor. Higher is snappier. |
| glow | number | 0.75 | Soft phosphor glow around the text dots (0 to 1). |
| aberration | number | 0.75 | Soft chromatic aberration toward the lens edge (0 to 1). |
