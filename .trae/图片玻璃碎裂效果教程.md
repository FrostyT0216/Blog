...
https://canvasui.dev/docs/components/shatter
...
const props = defineProps<ShatterOptions>();
...
| --- | --- | --- | --- |
| radius | number | 0.4 | Radius of the shatter lens around the cursor, relative to the screen height. |
| softness | number | 0.6 | Edge feather of the lens as a fraction of the radius (0 to 1). |
| tileSize | number | 125 | Tile size in CSS pixels. |
| shards | number | 1 | Shape irregularity. 0 keeps a perfect square grid, 1 breaks the page into uneven glass shards. |
| corner | number | 0 | Corner rounding of fully lifted tiles in CSS pixels. |
| lift | number | 30 | How high tiles lift off the page in CSS pixels. |
| tilt | number | 2 | How steeply tiles tip out of the page plane (0 to 3). |
| scatter | number | 5 | How far tiles slide sideways while lifted, in CSS pixels. |
| perspective | number | 1500 | Perspective distance in CSS pixels. Lower is more dramatic. |
| gapColor | [number, number, number] | [0, 0, 0] | Color of the void behind lifted tiles as [r, g, b] in 0-1 range. |
| shadow | number | 0.5 | Opacity of the drop shadows under lifted tiles (0 to 2). |
| shading | number | 0.5 | Strength of the per-tile lighting (0 to 2). |
| refraction | number | 1.5 | How strongly lifted shards refract the content beneath them, like glass (0 to 2). |
| dispersion | number | 0.3 | Chromatic fringing of the refraction (0 to 1). 0 keeps it color-true. |
| floatSpeed | number | 2 | Speed of the floating tile motion. 0 freezes the tiles. |
| strength | number | 1 | How fully tiles lift inside the lens (0 to 1). |
| baseStrength | number | 0 | Lift amount across the whole screen, outside the lens (0 to 1). |
| followSpeed | number | 3 | How quickly the lens follows the cursor. Higher is snappier. |
